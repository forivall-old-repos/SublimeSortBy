...
class SrtbyliCommand(sublime_plugin.TextCommand):
    def sortNumbers(self, edit, region, contenue, sort, estSelect, reversed):
    ...
    def sortStrings(self, edit, region, contenue, sort, estSelect, reversed):
        ...
        if sort == 'length':
            conteneur = sorted(
                contenue, key=lambda str: len(str), reverse=reversed)
        elif sort == 'string':
            conteneur = sorted(
                contenue, key=lambda str: str.lower(), reverse=reversed)
    ...
    def run(self, edit, sort='length', reversed=False):
        ...
        if view.sel()[0].empty() and len(view.sel()) == 1:  # No selection
            lines = view.substr(
                sublime.Region(0, self.view.size())).splitlines(True)

            region = view.sel()[-1].end()
            if sort in ('length', 'string', 'naturalOrder'):
                self.sortStrings(edit, region, lines, sort, False, reversed)
            elif sort in ('decimal', 'octal', 'hexadecimal', 'binary'):
                self.sortNumbers(edit, region, lines, sort, False, reversed)
            elif reversed:
                self.sortReverse(edit, region, lines, False)
        else:
            for region in view.sel():
                ...
                if region.empty():
                    ...
                    if sort in ('length', 'string', 'naturalOrder'):
                        self.sortStrings(
                            edit, region, lines, sort, True, reversed)
                    elif sort in ('decimal', 'octal', 'hexadecimal', 'binary'):
                        self.sortNumbers(
                            edit, region, lines, sort, True, reversed)
                    elif reversed:
